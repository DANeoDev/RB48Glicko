...
os.environ.setdefault("RB48_DATABASE_FILE", str(ROOT / "data" / "demo" / "demo.db"))
...
from scripts.simulation.simulation_analysis import build_simulation_table, build_player_analysis
...
@app.route("/simulation-analysis")
def simulation_analysis():
    selected_pitch = request.args.get("pitch", "total")
    if selected_pitch not in ("total", "box", "hf"):
        selected_pitch = "total"
    selected_player_id = request.args.get("player", "")

    connection = get_connection()
    players = get_players(connection)
    ratings = get_ratings(connection)
    table = build_simulation_table(players, ratings, selected_pitch)

    player_analysis = None
    if selected_player_id.isdigit():
        player_id = int(selected_player_id)
        if player_id in players:
            player_analysis = build_player_analysis(connection, player_id)

    connection.close()
    return render_template(
        "simulation_analysis.html",
        table=table,
        pitch=selected_pitch,
        players=players,
        selected_player_id=int(selected_player_id) if selected_player_id.isdigit() else None,
