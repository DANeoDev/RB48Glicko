...
def _quantile_baskets(predictions):
    ...
    for index in range(basket_count):
        ...
        baskets.append({"label": f"{min(predictions_only) * 100:.1f}–{max(predictions_only) * 100:.1f}%", "count": len(values), "predicted": sum(predictions_only) / len(values), "actual": sum(actuals) / len(values), "goal_diff_percentile": sum(goal_diff_percentiles) / len(values)})
    return baskets
...
def _lowess(predictions, value_key="actual", points=50, fraction=0.35):
    ...
    for step in range(points):
        ...
        if bandwidth == 0:
            ...
            weights = [(1 - (distance / bandwidth) ** 3) ** 3 if distance <= bandwidth else 0.0 for distance in distances]
...
def analyze_model(connection, mode=TOTAL, pitch=None):
    ...
    for match in matches.values():
        ...
        if any(player_id not in ratings or rating_type not in ratings[player_id] for player_id in team_a_ids + team_b_ids):
            excluded += 1
            continue
    ...
    if not count:
        return {"mode": mode, "pitch": pitch, "games": 0, "excluded": excluded, "brier": None, "log_loss": None, "mean_absolute_error": None, "accuracy": None, "calibration": [], "lowess": [], "goal_diff_lowess": [], "goal_diff_reference": {}}
    ...
    return {"mode": mode, "pitch": pitch, "games": count, "excluded": excluded, "brier": brier, "log_loss": log_loss, "mean_absolute_error": mean_absolute_error, "accuracy": accuracy, "calibration": _quantile_baskets(observations), "lowess": _lowess(observations, "actual"), "goal_diff_lowess": _lowess(observations, "goal_diff_percentile"), "goal_diff_reference": goal_diff_reference}
