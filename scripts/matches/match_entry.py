...
def _percentile(values, percentile):
    if not values: return DEFAULT_RATING
    values = sorted(values)
    if len(values) == 1: return values[0]
    position = (len(values) - 1) * percentile
    lower = int(position); upper = min(lower + 1, len(values) - 1); fraction = position - lower
    return values[lower] + (values[upper] - values[lower]) * fraction


def calibration_values(connection, level):
    if level not in CALIBRATION_LEVELS: raise ValueError("Invalid calibration level.")
    percentile, _ = CALIBRATION_LEVELS[level]
    if percentile is None: return {"rating": DEFAULT_RATING, "rd": DEFAULT_RD, "sigma": DEFAULT_SIGMA}
...
def create_new_player(connection, alias, positions=None, calibration_level="average"):
    ...
    if alias in get_alias_lookup(connection): raise ValueError(f"The alias '{alias}' already exists.")
    player_id = get_next_player_id(connection); create_player(connection, player_id); add_alias(connection, alias, player_id)
    for position in positions or []:
        if position.upper() in {"GK", "DEF", "MID", "ATT"}: add_position(connection, player_id, position.upper())
...
def next_match_id(connection, match_date):
    rows = connection.execute("SELECT match_id FROM matches WHERE date = ?", (match_date,)).fetchall(); highest = 0
    for row in rows:
...
def add_match(connection, match_date, pitch, team_a_ids, team_b_ids, goals_a, goals_b, players_a_count=None, players_b_count=None, match_id=None):
    ...
    try:
        ...
        for player_id in team_a_ids: add_match_player(connection, match_id, player_id, "a")
...
def process_new_matches(connection):
    calibrations = get_calibrations(connection); matches = get_matches(connection); processed_ids = get_processed_match_ids(connection)
    new_matches = sorted([m for m in matches.values() if m["match_id"] not in processed_ids], key=lambda m: m["match_id"])
    if not new_matches: return 0
    ratings = get_ratings(connection)
    if not ratings: raise RuntimeError("No current ratings found. Run the full Glicko calculation first.")
    rating_objects = glicko_table_to_ratings(ratings); engine = Glicko2()
    for match in new_matches:
        team_a, team_b = get_match_teams(connection, match["match_id"])
        for player_id in team_a + team_b: initialize_player_ratings(player_id, rating_objects, calibrations)
        write_match_ratings(connection, match["match_id"], ratings_to_glicko_table(rating_objects)); update_match(connection, match, rating_objects, engine)
    write_glicko(connection, ratings_to_glicko_table(rating_objects)); return len(new_matches)


def parse_uploaded_matches(file_bytes):
    reader = csv.reader(io.StringIO(file_bytes.decode("utf-8-sig"))); rows = []
    for line_number, row in enumerate(reader, start=1):
        if not row or all(not cell.strip() for cell in row): continue
        if len(row) < 6: raise ValueError(f"Line {line_number}: expected 6 columns (match_id, pitch, team A, team B, goals A, goals B).")
        rows.append(row[:6])
    return rows


def resolve_uploaded_match(row, alias_lookup, ignored_aliases):
    match_id, pitch, team_a_text, team_b_text, goals_a, goals_b = row; match_date = match_id.rsplit("-", 1)[0]
    if pitch not in ("box", "hf"): raise ValueError(f"{match_id}: pitch must be 'box' or 'hf'.")
    try: goals_a, goals_b = int(goals_a), int(goals_b)
    except ValueError: raise ValueError(f"{match_id}: goals must be integers.")
    def resolve_team(text):
        ids = []
        for alias in (name.strip() for name in text.split(",")):
            if not alias: continue
            if alias in alias_lookup: ids.append(alias_lookup[alias])
            elif alias in ignored_aliases: continue
            else: raise ValueError(f"{match_id}: unknown player alias '{alias}'.")
        return ids
    team_a, team_b = resolve_team(team_a_text), resolve_team(team_b_text)
    if set(team_a) & set(team_b): raise ValueError(f"{match_id}: a player appears on both teams.")
    if len(team_a) != len(set(team_a)) or len(team_b) != len(set(team_b)): raise ValueError(f"{match_id}: a player appears more than once.")
    return {"match_id": match_id, "date": match_date, "pitch": pitch, "team_a": team_a, "team_b": team_b, "goals_a": goals_a, "goals_b": goals_b, "players_a": len(team_a_text.split(",")) if team_a_text.strip() else 0, "players_b": len(team_b_text.split(",")) if team_b_text.strip() else 0}


def import_uploaded_matches(connection, file_bytes):
    rows = parse_uploaded_matches(file_bytes); alias_lookup = get_alias_lookup(connection); ignored_aliases = get_ignored_aliases(connection); imported = []
    for row in rows:
        parsed = resolve_uploaded_match(row, alias_lookup, ignored_aliases)
        if match_exists(connection, parsed["match_id"]): continue
        add_match(connection, parsed["date"], parsed["pitch"], parsed["team_a"], parsed["team_b"], parsed["goals_a"], parsed["goals_b"], parsed["players_a"], parsed["players_b"], parsed["match_id"])
